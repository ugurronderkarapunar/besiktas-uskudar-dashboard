...
try:
    ...
    # ------------------ YENİ SAYFALAR ------------------
        ...
        fig.add_trace(go.Scattermapbox(
            lon=[besiktas_lon], lat=[besiktas_lat],
            mode='markers+text',
            marker=dict(size=14, color='#e63946'),
            text=['Beşiktaş'], textposition='top right',
            name='Beşiktaş İskelesi'
        ))
        fig.add_trace(go.Scattermapbox(
            lon=[uskudar_lon], lat=[uskudar_lat],
            mode='markers+text',
            marker=dict(size=14, color='#457b9d'),
            text=['Üsküdar'], textposition='top left',
            name='Üsküdar İskelesi'
        ))
        # Marmaray Üsküdar
        fig.add_trace(go.Scattermapbox(
            lon=[29.0150], lat=[41.0250],
            mode='markers',
        ...
        fig.update_layout(
            mapbox=dict(style='open-street-map',
                        center=dict(lat=41.03495, lon=29.01095), zoom=12),
        ...
        st.markdown(
            ...
            'Turuncu üçgen Marmaray Üsküdar istasyonunu işaret eder.</div>',
            unsafe_allow_html=True,
        )
